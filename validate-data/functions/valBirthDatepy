...
def valBirthDate(input):
  ...
  # 1.2 Instantiates the variable with the date format that will be used to validate and format the data output.
  tgtFormat = "%Y-%m-%d"


  # 1.2 Traverses the data dictionary to validate the data regex structure and return the value of dict. 
  ...
  # 1.3 Converts the date format as defined in item 1.2
  try:
    dataObject  = datetime.strptime(input, srcFormt).strftime(tgtFormat)
  except ValueError:
    msgErro = '099'

  # 1.4 Convert string to datetime format. 
  d1 = datetime.strptime(dataObject,tgtFormat)
  d2 = datetime.today()

  # 1.5 calculates the number of days between two dates
  qtd = abs((d2 - d1).days)

  # 1.6 Realized logical test to validate the status os information. 
